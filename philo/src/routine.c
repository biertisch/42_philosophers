...
static void	think_until_hungry(t_philo *philo)
{
	...
	if (philo->sim->time_to_die <= philo->sim->time_to_eat + philo->sim->time_to_sleep + 20) // adjust if necessary
		threshold = philo->sim->time_to_eat + 5; //adjust if necessary, perhaps just sleep 1-5 ms
	threshold = philo->sim->time_to_die * 0.75; //adjust if necessary
...
{
	if (philo->id % 2 == 0)
		sleep_ms(philo->sim, philo->sim->time_to_eat); //adjust if necessary
	else
		sleep_ms(philo->sim, philo->sim->time_to_eat / 10); //adjust if necessary
}
